...
export default function TokenAnalytics({
  ...
  // ── Token pie data ──
  const thinkingTokens = thinkingTokensProp || Math.max(0, totalTokens - promptTokens - completionTokens);
  const tokenPieData = [
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-5 gap-2 mb-3">
        ...
        <div className="glass-card-sm p-2.5 text-center">
          <div className="text-[10px] text-av-muted uppercase">Prompt</div>
          <div className="text-lg font-mono font-bold text-sky-400">{promptTokens.toLocaleString()}</div>
          <div className="text-[9px] text-av-muted mt-0.5">input tokens</div>
        </div>
        <div className="glass-card-sm p-2.5 text-center">
          <div className="text-[10px] text-av-muted uppercase">Completion</div>
          <div className="text-lg font-mono font-bold text-emerald-400">{completionTokens.toLocaleString()}</div>
          <div className="text-[9px] text-av-muted mt-0.5">output tokens</div>
        </div>
        <div className="glass-card-sm p-2.5 text-center">
          <div className="text-[10px] text-av-muted uppercase">Total Time</div>
          <div className="text-lg font-mono font-bold text-av-warning">{totalTime.toFixed(1)}s</div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    formatter={(value, name) => [
                      `${value.toLocaleString()} (${((value / totalTokens) * 100).toFixed(1)}%)`,
                      name,
                    ]}
                  />
                  <Legend
