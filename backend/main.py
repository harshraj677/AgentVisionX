...
async def execute_query(request: QueryRequest):
    ...
    _run_task(engine.execute(request.query))
...
async def execute_with_failure(request: QueryRequest):
    ...
    # Get step_id from request body if provided
    failure_step_id = getattr(request, 'step_id', None)
    if not failure_step_id and hasattr(request, '__dict__'):
        failure_step_id = request.__dict__.get('step_id')
    _run_task(engine.execute(request.query, failure_step_id=failure_step_id))
